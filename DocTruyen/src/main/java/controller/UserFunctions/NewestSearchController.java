...
/**
 * Servlet implementation class NewestSearchController
 */
@WebServlet("/newest")
public class NewestSearchController extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private StoryDAO storyDAO = new StoryDAO();
	private ChapterDAO chapterDAO = new ChapterDAO();

	public NewestSearchController() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		List<Stories> stories = storyDAO.getNewestStories();
		...
		for (Stories s : stories) {
			...
			if (latest != null && latest.getPublishedAt() != null && maxChap > 0) {
				latestChapterInfo.put(s.getId(), "C. " + maxChap + " - " + s.getTimeAgo());
			} else {
		...
		request.setAttribute("subTitle", "Truyện mới nhất !");
		request.setAttribute("stories", stories);
		request.setAttribute("latestChapterInfo", latestChapterInfo);

		request.getRequestDispatcher("/WEB-INF/view/utility/record_and_tag.jsp").forward(request, response);
	}
